...
Ltac unpack :=
  match goal with
  | [ s : kstate, H : ?s = _,
      s' : kstate, H' : ?s' = kstate_run_prog _ _ _ _ ?s _ |- _ ]
      => idtac "1"; rewrite H in H'; apply f_equal with (f:=ktr) in H';
         simpl in H'; rewrite H' in *; unpack
  | [ s : init_state _ _, H : ?s = _ |- _ ]
      (*This is problematic because it destroys information about s*)
      => idtac "2"; rewrite H in *; simpl in *; unpack
  | [ H: [_]%inhabited = [_]%inhabited |- _ ] =>
    idtac "3"; apply pack_injective in H;
