...
(*Ltac unpack :=
  match goal with
  | [ H : InitialState _ _ _ _ _ _ _ _ ?s |- _ ]
    => inversion H;
       match goal with
       | [ _ : ?s' = init_state_run_cmd _ _ _ _ _ _ _ _ _ |- _ ]
         => subst s'; subst s
       end
  | [ H : ValidExchange _ _ _ _ _ _ _ _ _ _ ?s |- _ ]
    => destruct_msg; destruct_comp; inversion H;
       match goal with
       | [ _ : ?s' = mk_inter_ve_st _ _ _ _ _ _ _ _ |- _ ]
         => subst s'; subst s
       end
  | [ H : BogusExchange _ _ _ _ _ _ _ ?s |- _ ]
    => inversion H; subst s
  end; simpl; try destruct_ite; simpl; intros; uninhabit.*)
  ...
  | [ Htr : _ = inhabits ?tr |- _ ]
      => match goal with
         ...
         | [ _ : Reach _ _ _ _ _ _ _ ?HDLRS _,
             H : ?s = _,
             H' : ?s' = kstate_run_prog _ _ _ _ _ _ _ _ ?s _ _,
             input : kstate_run_prog_return_type _ _ _ _ _ _ _ _ _ _ |- _ ]
...
Ltac use_IH_disables :=
  match goal with
  | [ IHReach : context[forall tr' : KTrace _ _ _, _],
      _ : ktr _ _ _ _ ?s = inhabits ?tr |- _ ]
...
Ltac match_disables :=
  ...
  | [ |- Disables ?pdv ?compt ?comps ?comptdec _ ?future (?act::_) ]
      => let s := match goal with
         ...
         let H := fresh "H" in
         pose proof (decide_act compt comps comptdec pdv future act) as H;
         destruct H;
         [ contradiction ||
...
Lemma cut_exists : forall nb_msg plt compt comps act disj_R conj_R,
  (exists past : @KAction nb_msg plt compt comps, (disj_R past) /\ (conj_R past)) ->
...
Ltac releaser_match :=
  simpl;
  repeat match goal with
         | [ |- exists past : KAction _ _ _, (?act = _ \/ ?disj_R ) /\ ?conj_R ]
...
Ltac use_IH_releases :=
  match goal with
  | [ IHReach : context[ exists past : KAction _ _ _, _ ] |- _ ]
...
Ltac exists_past :=
  ...
  reach_induction;
  match goal with
  | [ _ : InitialState _ _ _ _ _ _ _ _ _ |- _ ]
    => try subst; simpl in *;
       try solve [ impossible
                 | releaser_match ]
  | [ _ : ValidExchange _ _ _ _ _ _ _ _ _ _ _ |- _ ]
    => try subst; simpl in *;
       try solve [ impossible
                 | use_IH_releases
                 | releaser_match
  ...
  | [ _ : BogusExchange _ _ _ _ _ _ _ _ |- _ ]
