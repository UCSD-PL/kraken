...
Notation CookieProcessRegister := 14%fin (only parsing).
Notation AddrFocus := 15%fin (only parsing).
Notation DomainSet := 16%fin (only parsing).
...
Definition HANDLERS : handlers PAYD COMPT COMPS KSTD :=
  ...
  | Tab, CookieChannelInit =>
      let envd := mk_vcdesc [Desc _ fd_d; Comp _ CProc] in
      let envd' := mk_vcdesc [Desc _ fd_d; Comp _ CProc; Comp _ CProc] in
      [[ envd :
        seq (call _ envd (slit (str_of_string create_ckchan)) [] 0%fin (Logic.eq_refl _))
       (seq (send ccomp CookieProcessRegister (envvar envd 0%fin, tt))
            (complkup (envd:=envd) (mk_comp_pat _ _ CProc (Some hdlr_tab_dom, tt))
                 (send (envvar envd' 2%fin) TabProcessRegister (envvar envd' 0%fin, tt))
                 (seq (spawn _ envd CProc (hdlr_tab_dom, tt) 1%fin
                             (Logic.eq_refl _))
                 (seq (send (envvar envd 1%fin) DomainSet (hdlr_tab_dom, tt))
                      (send (envvar envd 1%fin) TabProcessRegister
                            (envvar envd 0%fin, tt))))))
      ]]
  | _, _ =>
    [[ mk_vcdesc [] : nop ]]
  end.
