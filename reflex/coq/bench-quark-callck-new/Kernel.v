...
Notation TabProcessRegister := 12%fin (only parsing).
Notation CookieProcessRegister := 13%fin (only parsing).
...
Definition create_ckchan := "doesnotexist".
...
Definition HANDLERS : handlers PAYD COMPT COMPS KSTD :=
  ...
  | UserInput, TabCreate =>
      ...
      let envd' := mk_vcdesc [Comp _ Tab; Desc _ fd_d; Comp _ CProc; Comp _ CProc] in
      [[ envd :
          complkup (envd:=envd)
                   (mk_comp_pat _ _ Tab (Some (mvar TabCreate 0%fin), (None, tt)))
                   nop
                   (seq (spawn _ envd Tab (mvar TabCreate 0%fin, (mvar TabCreate 1%fin, tt))
                               None (Logic.eq_refl _))
                   (seq (send (envvar envd 0%fin) DomainSet (mvar TabCreate 1%fin, tt))
                   (seq (call _ envd (slit (str_of_string create_ckchan)) []
                              1%fin (Logic.eq_refl _))
                   (seq (send (envvar envd 0%fin) CookieProcessRegister (envvar envd 1%fin, tt))
                   (seq (complkup (envd:=envd)
                                  (mk_comp_pat _ _ CProc (Some (mvar TabCreate 1%fin), tt))
                                  (send (envvar envd' 3%fin)
                                        TabProcessRegister (envvar envd' 1%fin, tt))
                                  (seq (spawn _ envd CProc (mvar TabCreate 1%fin, tt)
                                              2%fin (Logic.eq_refl _)) (
                                       (send (envvar envd 2%fin) TabProcessRegister
                                             (envvar envd 1%fin, tt)))))
