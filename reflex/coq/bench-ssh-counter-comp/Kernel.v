...
Inductive COMPT' : Type := System | Slave | Counter.
...
Definition COMPS (t : COMPT) : compd :=
  ...
  | Slave  => mk_compd
                ...
                (mk_vdesc [])
  | Counter  => mk_compd
                "Counter"  "/home/don/kraken/kraken/ssh-proto/kmsg-ssh/ssh"      []
                (mk_vdesc [])
  end.
...
Notation CLoginReq       := 18%fin (only parsing).
Notation CLoginResT      := 19%fin (only parsing).
Notation CLoginResF      := 20%fin (only parsing).

Definition IENVD : vcdesc COMPT := mk_vcdesc
  [ Comp _ System; Comp _ Slave; Comp _ Counter ].

Notation v_env_system  := (None) (only parsing).
Notation v_env_slave   := (Some None) (only parsing).
Notation v_env_counter := (Some (Some None)) (only parsing).
...
Notation v_st_counter       := 2%fin (only parsing).
...
Definition INIT : init_prog PAYD COMPT COMPS KSTD IENVD :=
  ...
  (seq (spawn _ IENVD Counter  tt                         v_env_counter  (Logic.eq_refl _))
       (stupd _ IENVD v_st_counter (i_envvar IENVD v_env_counter)))))).
...
Definition HANDLERS : handlers PAYD COMPT COMPS KSTD :=
  ...
  with
     | Slave, LoginReq =>
       [[ mk_vcdesc [] :
            send (stvar v_st_counter) CLoginReq (mvar LoginReq 0%fin, tt)
       ]]
     | Counter, CLoginResT =>
       [[ mk_vcdesc [] :
          send (stvar v_st_system) SLoginReq (mvar CLoginResT 0%fin, tt)
       ]]
     | System, SLoginResT =>
       [[ mk_vcdesc [] :
