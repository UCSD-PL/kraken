...
Ltac match_releases :=
  ...
  intros. Time generalize dependent tr; induction H.
Time unpack. match_releases.
Time unpack.
  admit.
  admit.
  admit.
...
match_releases.
(*Time inversion H0; subst s'0; subst s'; clear H0;
  simpl; unfold kstate_run_prog in *; simpl in *;
  repeat destruct_find_comp; repeat destruct_cond;
       simpl in *; intros; try uninhabit; match_releases.*)
idtac "About to find a match";
  extract_match_facts;
  (*There may be conditions on s' (the intermediate state). We want
    to use these conditions to derive conditions on s.*)
  (*subst_states;*)
  ...
  (*Should this take s as an argument?*)
  generalize dependent tr0; induction H.
    Time unpack;
  try solve [ idtac "impossible"; impossible
            | idtac "IH_releases"; use_IH_releases
            | idtac "releaser match"; releaser_match
            | idtac "auto"; auto].
    Time unpack;
  try solve [ idtac "impossible"; impossible
            | idtac "IH_releases"; use_IH_releases
...
Time inversion H0; subst s'0; clear H0;
  simpl; unfold kstate_run_prog in *; simpl in *; subst s';
  repeat destruct_find_comp; repeat destruct_cond;
...
(*Time inversion H0; subst s'0; subst s'; clear H0;
  ...
  repeat destruct_find_comp; repeat destruct_cond;
       simpl in *; intros; try uninhabit.*)
Time inversion H0; subst s'0; clear H0; revert H7;
  simpl; unfold kstate_run_prog; simpl;
  repeat destruct_find_comp; repeat destruct_cond; intros; subst s';
       simpl in *; intros; try uninhabit.



  Time inversion H0. Time subst s'0; subst s'. clear H0.
   Time simpl. Time unfold kstate_run_prog in *. Time simpl in *.
   Time repeat destruct_find_comp.
       idtac "destructing cond"; repeat destruct_cond; idtac "about to simpl in *";
       simpl in *; intros; idtac "about to uninhabit"; try uninhabit.
  

inversion_clear H. unpack. unfold eval_base_payload_expr, eval_payload_expr. simpl.
  Time crush.
Qed.*)
