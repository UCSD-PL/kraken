...
Definition INIT : init_prog PAYD COMPT COMPS KSTD IENVD :=
   spawn _ IENVD C (i_nlit (num_of_nat 0), tt) v_env_c (Logic.eq_refl _).
...
Definition HANDLERS : handlers PAYD COMPT COMPS KSTD :=
  ...
  with
  | M, C =>
    let envd := mk_vcdesc [Comp _ C] in
    [[ envd :
       complkup (mk_comp_pat _ _ C (Some (mvar M 0%fin), tt))
                (
                  nop
                )
                (
                  spawn _ envd C (mvar M 0%fin, tt) 0%fin (Logic.eq_refl _)
                )
    ]]
  | _, _ => [[ mk_vcdesc [] : nop ]]
  end.
