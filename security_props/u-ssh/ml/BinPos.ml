...
(** val coq_Pmult_nat : positive -> MlCoq.nat -> MlCoq.nat **)
...
(** val nat_of_P : positive -> MlCoq.nat **)

let nat_of_P x =
  coq_Pmult_nat x (MlCoq.S MlCoq.O)

(** val coq_P_of_succ_nat : MlCoq.nat -> positive **)

let rec coq_P_of_succ_nat = function
| MlCoq.O -> Coq_xH
| MlCoq.S x -> coq_Psucc (coq_P_of_succ_nat x)
...
(** val coq_Psize : positive -> MlCoq.nat **)

let rec coq_Psize = function
| Coq_xI p0 -> MlCoq.S (coq_Psize p0)
| Coq_xO p0 -> MlCoq.S (coq_Psize p0)
| Coq_xH -> MlCoq.S MlCoq.O
